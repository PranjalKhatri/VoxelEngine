...
TerrainGenerator::TerrainGenerator(Properties properties)
    : properties_(properties) {
    SetupNoise();
}
void TerrainGenerator::SetupNoise() {
    noise_.SetSeed(properties_.seed);
    ...
    noise_.SetFrequency(properties_.frequency);
}
float TerrainGenerator::GetDensity(float x, float y, float z) {
    float n3d = noise_.GetNoise(x, y, z);

    float heightGradient = (y - properties_.heightBias) / properties_.hardness;
